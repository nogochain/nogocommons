...
func (a *AddrManager) updateAddress(netAddr, srcAddr *wire.NetAddressV2) {
	...
	ka := a.find(netAddr)
	if ka != nil {
		// Known addresses are updated with the latest last-seen time
		// and services on each contact. Periodic batching could reduce
		// write overhead but is deferred for now.
