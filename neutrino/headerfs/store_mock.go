...
//
//nolint:lll
func (m *MockBlockHeaderStore) LatestBlockLocator() (BlockLocator, error) {
	args := m.Called()
	return args.Get(0).(BlockLocator), args.Error(1)
}
...
func (m *MockWalletDB) View(fn func(tx walletdb.ReadTx) error, reset func()) error {
	args := m.Called(fn, reset)
	return args.Error(0)
}
