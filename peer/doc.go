...
// Copyright (c) 2024 The nogochain developers
// Use of this source code is governed by an ISC
// license that can be found in the LICENSE file.

/*
Package peer provides a common base for creating and managing NogoChain network
peers.

# Overview

This package builds upon the wire package, which provides the fundamental
primitives necessary to speak the NogoChain P2P protocol, in order to simplify
...
All communication uses the BIP324 v2 encrypted transport protocol exclusively.
There is no v1 plaintext fallback. Every connection is encrypted end-to-end
using the v2 transport handshake with ElligatorSwift key exchange and
ChaCha20-Poly1305 authenticated encryption.

A quick overview of the major features peer provides are as follows:

  - Provides a basic concurrent safe peer for handling P2P communications via
    the peer-to-peer protocol with BIP324 v2 transport encryption
  - Full duplex reading and writing of protocol messages over encrypted channels
  - Automatic handling of the initial v2 handshake process including protocol
    version negotiation
  ...
  - Flexible peer configuration
    ...
    3. NogoChain network parameters
    4. Service support signalling (full nodes, compact filters, etc)
    5. Maximum supported protocol version
    6. Ability to register callbacks for handling protocol messages
...
caller to specify things such as the user agent name and version, the NogoChain
network to use, which services it supports, and callbacks to invoke when
protocol messages are received. See the documentation for each field of the
Config struct for more details.
...
and initiate the BIP324 v2 protocol negotiation process. Once finished with the
peer call Disconnect to disconnect from the peer and clean up all resources.
...
protocol messages. This is accomplished by creating an instance of the
MessageListeners struct with the callbacks to be invoked specified and setting
the Listeners field of the Config struct specified when creating a peer to it.

For convenience, a callback hook for all of the currently supported protocol
