...
func NewWIF(privKey *nogoec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + nogoec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + nogoec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+nogoec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+nogoec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+nogoec.PrivKeyBytesLen]
	privKey, _ := nogoec.PrivKeyFromBytes(privKeyBytes)
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + nogoec.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
