...
func TestNewTxFromBytes(t *testing.T) {
	...
	tx, err := nogoutil.NewTxFromBytes(testTxBytes)
	if err != nil {
...
func TestTxErrors(t *testing.T) {
	...
	_, err = nogoutil.NewTxFromBytes(shortBytes)
	if err != io.EOF {
...
func TestTxHasWitness(t *testing.T) {
	...
	for _, msgTxWithoutWitness := range Block100000.Transactions[1:] {
		txWithoutWitness := nogoutil.NewTx(msgTxWithoutWitness)
