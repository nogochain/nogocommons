...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		_, pubKey := nogoec.PrivKeyFromBytes(k.key)
...
func (k *ExtendedKey) Derive(i uint32) (*ExtendedKey, error) {
	...
	if k.isPrivate {
		...
		var (
			ilScalar nogoec.ModNScalar
			ilJ      nogoec.JacobianPoint
		)
		if overflow := ilScalar.SetByteSlice(il); overflow {
			return nil, ErrInvalidChild
		}
		nogoec.ScalarBaseMultNonConst(&ilScalar, &ilJ)
		...
		// public key.
		pubKey, err := nogoec.ParsePubKey(k.key)
		if err != nil {
			return nil, err
		}
		...
		var pubKeyJ nogoec.JacobianPoint
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		var childKeyPubJ nogoec.JacobianPoint
		nogoec.AddNonConst(&ilJ, &pubKeyJ, &childKeyPubJ)
		...
		childKeyPub := nogoec.NewPublicKey(
...
func (k *ExtendedKey) DeriveNonStandard(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(nogoec.S256().N) >= 0 || ilNum.Sign() == 0 {
		return nil, ErrInvalidChild
	}
...
func (k *ExtendedKey) CloneWithVersion(version []byte) (*ExtendedKey, error) {
	if len(version) != 4 {
		// The expected error type is ErrInvalidHDKeyID. This returns
		// a generic error for now; a future chaincfg update may provide
		// the dedicated error type for more precise diagnostics.
...
func (k *ExtendedKey) ECPubKey() (*nogoec.PublicKey, error) {
	return nogoec.ParsePubKey(k.pubKeyBytes())
}
...
func (k *ExtendedKey) ECPrivKey() (*nogoec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := nogoec.PrivKeyFromBytes(k.key)
	return privKey, nil
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(nogoec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(nogoec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := nogoec.ParsePubKey(keyData)
		if err != nil {
			return nil, err
		}
	}
