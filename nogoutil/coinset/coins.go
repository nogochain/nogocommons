...
func (cs *CoinSet) TotalValue() (value nogoutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue nogoutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue nogoutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount nogoutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue nogoutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue nogoutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue nogoutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue nogoutil.Amount, coins []Coin) (Coins, error) {
...
// nogoutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() nogoutil.Amount {
	return nogoutil.Amount(c.txOut().Value)
}
...
// ExtractPkScriptAddrs for example.
