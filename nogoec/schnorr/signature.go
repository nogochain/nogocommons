...
func NewSignature(r *nogoec.FieldVal, s *nogoec.ModNScalar) *Signature {
	var sig Signature
...
func schnorrVerify(sig *Signature, hash []byte, pubKeyBytes []byte) error {
	...
	//
	// R = s*G - e*P
	var P, R, sG, eP nogoec.JacobianPoint
	pubKey.AsJacobian(&P)
	nogoec.ScalarBaseMultNonConst(&sig.s, &sG)
	nogoec.ScalarMultNonConst(&e, &P, &eP)
	nogoec.AddNonConst(&sG, &eP, &R)
...
func (sig *Signature) Verify(hash []byte, pubKey *nogoec.PublicKey) bool {
...
func schnorrSign(privKey, nonce *nogoec.ModNScalar, pubKey *nogoec.PublicKey, hash []byte,
	...
	//
	// R = kG
	var R nogoec.JacobianPoint
	k := *nonce
	nogoec.ScalarBaseMultNonConst(&k, &R)
	...
	//
	// s = k + e*d mod n
	s := new(nogoec.ModNScalar).Mul2(&e, privKey).Add(&k)
	k.Zero()
...
func Sign(privKey *nogoec.PrivateKey, hash []byte,
	...
	//
	// d' = int(d)
	var privKeyScalar nogoec.ModNScalar
