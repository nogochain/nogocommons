...
// nogoec.Publickey, verifying that it is valid. It only supports public keys in
// the BIP-340 32-byte format.
func ParsePubKey(pubKeyStr []byte) (*nogoec.PublicKey, error) {
	if pubKeyStr == nil {
	...
	var keyCompressed [nogoec.PubKeyBytesLenCompressed]byte
	keyCompressed[0] = secp.PubKeyFormatCompressedEven
	copy(keyCompressed[1:], pubKeyStr)

	return nogoec.ParsePubKey(keyCompressed[:])
}
...
// coordinate.
func SerializePubKey(pub *nogoec.PublicKey) []byte {
