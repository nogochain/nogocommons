...
func secNonceToPubNonce(secNonce [SecNonceSize]byte) [PubNonceSize]byte {
	var k1Mod, k2Mod nogoec.ModNScalar
	k1Mod.SetByteSlice(secNonce[:nogoec.PrivKeyBytesLen])
	k2Mod.SetByteSlice(secNonce[nogoec.PrivKeyBytesLen:])

	var r1, r2 nogoec.JacobianPoint
	nogoec.ScalarBaseMultNonConst(&k1Mod, &r1)
	nogoec.ScalarBaseMultNonConst(&k2Mod, &r2)

	// Next, we'll convert the key in jacobian format to a normal public
	// key expressed in affine coordinates.
	r1.ToAffine()
	r2.ToAffine()
	r1Pub := nogoec.NewPublicKey(&r1.X, &r1.Y)
	r2Pub := nogoec.NewPublicKey(&r2.X, &r2.Y)
...
// generation.
func WithPublicKey(pubKey *nogoec.PublicKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
func WithNonceSecretKeyAux(secKey *nogoec.PrivateKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
// generate nonces.
func WithNonceCombinedKeyAux(combinedKey *nogoec.PublicKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
// *nogoec.PrivateKey. This is mainly used to match the testcases provided by
// the MuSig2 BIP.
...
func GenNonces(options ...NonceGenOption) (*Nonces, error) {
	...
	k2Mod.PutBytesUnchecked(nonces.SecNonce[nogoec.PrivKeyBytesLen:])
	copy(nonces.SecNonce[nogoec.PrivKeyBytesLen*2:], opts.publicKey)
...
func AggregateNonces(pubNonces [][PubNonceSize]byte) ([PubNonceSize]byte, error) {
	...
	combineNonces := func(slicer nonceSlicer) (nogoec.JacobianPoint, error) {
		...
		var aggregateNonce nogoec.JacobianPoint
	...
	combinedNonce1, err := combineNonces(func(n [PubNonceSize]byte) []byte {
		return n[:nogoec.PubKeyBytesLenCompressed]
	})
	if err != nil {
	...
	copy(finalNonce[:], nogoec.JacobianToByteSlice(combinedNonce1))
	copy(
		finalNonce[nogoec.PubKeyBytesLenCompressed:],
		nogoec.JacobianToByteSlice(combinedNonce2),
	)
