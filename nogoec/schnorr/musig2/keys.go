...
type sortableKeys []*nogoec.PublicKey
...
// serialization.
func sortKeys(keys []*nogoec.PublicKey) []*nogoec.PublicKey {
...
func keyHashFingerprint(keys []*nogoec.PublicKey, sort bool) []byte {
	if sort {
		keys = sortKeys(keys)
	}
...
func keyBytesEqual(a, b *nogoec.PublicKey) bool {
...
func aggregationCoefficient(keySet []*nogoec.PublicKey,
	targetKey *nogoec.PublicKey, keysHash []byte,
	secondKeyIdx int) *nogoec.ModNScalar {

	var mu nogoec.ModNScalar
...
func secondUniqueKeyIndex(keySet []*nogoec.PublicKey, sort bool) int {
	if sort {
...
func hasEvenY(pJ nogoec.JacobianPoint) bool {
	pJ.ToAffine()
	p := nogoec.NewPublicKey(&pJ.X, &pJ.Y)
...
// tweak or not.
func tweakKey(keyJ nogoec.JacobianPoint, parityAcc nogoec.ModNScalar, tweak [32]byte,
	tweakAcc nogoec.ModNScalar,
	xOnly bool) (nogoec.JacobianPoint, nogoec.ModNScalar, nogoec.ModNScalar, error) {
	...
	var parityFactor nogoec.ModNScalar
	...
	// manipulations below.
	tweakInt := new(nogoec.ModNScalar)
	...
	//
	// First compute t*G:
	var tweakedGenerator nogoec.JacobianPoint
	nogoec.ScalarBaseMultNonConst(tweakInt, &tweakedGenerator)

	// Next compute g*Q:
	nogoec.ScalarMultNonConst(&parityFactor, &keyJ, &keyJ)

	// Finally add both of them together to get our final
	// tweaked point.
	nogoec.AddNonConst(&tweakedGenerator, &keyJ, &keyJ)
...
func AggregateKeys(keys []*nogoec.PublicKey, sort bool,
	keyOpts ...KeyAggOption) (
	*AggregateKey, *nogoec.ModNScalar, *nogoec.ModNScalar, error) {
	...
	finalKeyJ.ToAffine()
	combinedKey := nogoec.NewPublicKey(&finalKeyJ.X, &finalKeyJ.Y)
	...
	if opts.taprootTweak {
		// Emulate the same behavior as ComputeTaprootOutputKey
	...
	finalKeyJ.ToAffine()
	finalKey := nogoec.NewPublicKey(&finalKeyJ.X, &finalKeyJ.Y)
