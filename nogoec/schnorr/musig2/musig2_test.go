...
func (s signerSet) keys() []*nogoec.PublicKey {
	keys := make([]*nogoec.PublicKey, len(s))
	for i := 0; i < len(s); i++ {
		keys[i] = s[i].pubKey
	}
...
func (s signerSet) combinedKey() *nogoec.PublicKey {
...
func testMultiPartySign(t *testing.T, taprootTweak []byte,
	...
	signerKeys := make([]*nogoec.PrivateKey, numSigners)
	signSet := make([]*nogoec.PublicKey, numSigners)
	...
	var combinedKey *nogoec.PublicKey
...
func TestMuSigEarlyNonce(t *testing.T) {
	...
	privKey1, err := nogoec.NewPrivateKey()
	if err != nil {
		t.Fatalf("unable to gen priv key: %v", err)
	}
	privKey2, err := nogoec.NewPrivateKey()
	if err != nil {
		t.Fatalf("unable to gen priv key: %v", err)
	}
	...
	signers := []*nogoec.PublicKey{privKey1.PubKey(), privKey2.PubKey()}
...
func TestSigningWithAggregatedNonce(t *testing.T) {
	...
	t.Run("error: register combined nonce twice", func(t *testing.T) {
		privKey, _ := nogoec.NewPrivateKey()
		privKey2, _ := nogoec.NewPrivateKey()
		signSet := []*nogoec.PublicKey{privKey.PubKey(), privKey2.PubKey()}
	...
	t.Run("error: register combined nonce after register pub nonce",
		func(t *testing.T) {
			...
			privKey, _ := nogoec.NewPrivateKey()
			privKey2, _ := nogoec.NewPrivateKey()
			privKey3, _ := nogoec.NewPrivateKey()
			signSet := []*nogoec.PublicKey{
				privKey.PubKey(),
				privKey2.PubKey(),
				privKey3.PubKey(),
			}
