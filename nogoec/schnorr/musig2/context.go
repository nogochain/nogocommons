...
type Context struct {
	...
	signingKey *nogoec.PrivateKey

	// pubKey is our even-y coordinate public  key.
	pubKey *nogoec.PublicKey
...
func WithKnownSigners(signers []*nogoec.PublicKey) ContextOption {
	return func(o *contextOptions) {
		o.keySet = signers
...
func NewContext(signingKey *nogoec.PrivateKey, shouldSort bool,
	...
	case opts.numSigners != 0:
		...
		// be created.
		opts.keySet = make([]*nogoec.PublicKey, 0, opts.numSigners)
...
func (c *Context) RegisterSigner(pub *nogoec.PublicKey) (bool, error) {
...
func (c *Context) CombinedKey() (*nogoec.PublicKey, error) {
...
func (c *Context) PubKey() nogoec.PublicKey {
	return *c.pubKey
}

// SigningKeys returns the set of keys used for signing.
func (c *Context) SigningKeys() []*nogoec.PublicKey {
	keys := make([]*nogoec.PublicKey, len(c.opts.keySet))
...
func (c *Context) TaprootInternalKey() (*nogoec.PublicKey, error) {
	// If the caller hasn't registered all the signers at this point, then
	// the combined key won't be available.
	if c.combinedKey == nil {
		return nil, ErrNotEnoughSigners
...
func (s *Session) RegisterCombinedNonce(
	...
	// ensure it's valid.
	_, err := nogoec.ParsePubKey(combinedNonce[:33])
	if err != nil {
