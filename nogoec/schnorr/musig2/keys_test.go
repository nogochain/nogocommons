...
func TestMusig2KeySort(t *testing.T) {
	...
	keys := make([]*nogoec.PublicKey, len(testCase.PubKeys))
	for i, keyStr := range testCase.PubKeys {
		pubKey, err := nogoec.ParsePubKey(mustParseHex(keyStr))
		require.NoError(t, err)
	...
	expectedKeys := make([]*nogoec.PublicKey, len(testCase.PubKeys))
...
func keysFromIndices(t *testing.T, indices []int,
	pubKeys []string) ([]*nogoec.PublicKey, error) {

	t.Helper()

	inputKeys := make([]*nogoec.PublicKey, len(indices))
	for i, keyIdx := range indices {
		var err error
		inputKeys[i], err = nogoec.ParsePubKey(
...
func TestMuSig2TweakTestVectors(t *testing.T) {
	...
	privKey, _ := nogoec.PrivKeyFromBytes(mustParseHex(testCases.PrivKey))
