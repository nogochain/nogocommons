...
//
// TODO: Rewrite these to avoid new(nogoec.FieldVal).Add(...) usage?
// NOTE: u, t MUST be normalized. The result x is normalized.
func XSwiftEC(u, t *nogoec.FieldVal) (*nogoec.FieldVal, error) {
	...
	// Calculate g(u').
	gu := new(nogoec.FieldVal).SquareVal(u).Mul(u).AddInt(7).Normalize()

	// Calculate the right-hand side of the equation (-t'^2)
	rhs := new(nogoec.FieldVal).SquareVal(t).Negate(1).Normalize()
	...
	// 4. X = (u'^3 + b - t''^2) / (2t'')
	tSquared := new(nogoec.FieldVal).SquareVal(t).Negate(1)
	xNum := new(nogoec.FieldVal).SquareVal(u).Mul(u).AddInt(7).Add(tSquared)
	xDenom := new(nogoec.FieldVal).Add2(t, t).Inverse()
	x := xNum.Mul(xDenom)

	// 5. Y = (X+t'') / (u' * c)
	yNum := new(nogoec.FieldVal).Add2(x, t)
	yDenom := new(nogoec.FieldVal).Mul2(u, &c).Inverse()
	y := yNum.Mul(yDenom)
	...
	ySqr := new(nogoec.FieldVal).Add(y).Mul(y)
	quadYSqr := new(nogoec.FieldVal).Add(ySqr).MulInt(4)
	firstX := new(nogoec.FieldVal).Add(u).Add(quadYSqr)
	...
	doubleYInv := new(nogoec.FieldVal).Add(y).Add(y).Inverse()
	xDivDoubleYInv := new(nogoec.FieldVal).Add(x).Mul(doubleYInv)
	negXDivDoubleYInv := new(nogoec.FieldVal).Add(xDivDoubleYInv).Negate(1)
	invTwo := new(nogoec.FieldVal).AddInt(2).Inverse()
	negUDivTwo := new(nogoec.FieldVal).Add(u).Mul(invTwo).Negate(1)
	secondX := new(nogoec.FieldVal).Add(negXDivDoubleYInv).Add(negUDivTwo)
	...
	thirdX := new(nogoec.FieldVal).Add(xDivDoubleYInv).Add(negUDivTwo)
...
// x-coordinate.
func isXOnCurve(x *nogoec.FieldVal) bool {
	y := new(nogoec.FieldVal).Add(x).Square().Mul(x).AddInt(7)
	return new(nogoec.FieldVal).SquareRootVal(y)
}
...
func XSwiftECInv(u, x *nogoec.FieldVal, caseNum int) *nogoec.FieldVal {
	v := new(nogoec.FieldVal)
	s := new(nogoec.FieldVal)
	twoInv := new(nogoec.FieldVal).AddInt(2).Inverse()

	if caseNum&2 == 0 {
		// If lift_x(-x-u) succeeds, return None
		_, found := liftX(new(nogoec.FieldVal).Add(x).Add(u).Negate(2))
		if found {
			return nil
		}
		...
		uSqr := new(nogoec.FieldVal).Add(u).Square()
		vSqr := new(nogoec.FieldVal).Add(v).Square()
		sDenom := new(nogoec.FieldVal).Add(u).Mul(v).Add(uSqr).Add(vSqr)
		sNum := new(nogoec.FieldVal).Add(uSqr).Mul(u).AddInt(7)

		s = sDenom.Inverse().Mul(sNum).Negate(1)
	} else {
		// Let s = x - u
		negU := new(nogoec.FieldVal).Add(u).Negate(1)
		...
		uSqr := new(nogoec.FieldVal).Add(u).Square()
		lhs := new(nogoec.FieldVal).Add(uSqr).Mul(u).AddInt(7).MulInt(4)
		rhs := new(nogoec.FieldVal).Add(uSqr).MulInt(3).Mul(s)

		// Add the two terms together and multiply by -s.
		lhs.Add(rhs).Normalize().Mul(s).Negate(1)

		r := new(nogoec.FieldVal)
		...
		// Let v = (r/s - u)/2
		sInv := new(nogoec.FieldVal).Add(s).Inverse()
		uNeg := new(nogoec.FieldVal).Add(u).Negate(1)

		v.Add(r).Mul(sInv).Add(uNeg).Mul(twoInv)
	}

	w := new(nogoec.FieldVal)
	...
	case 0:
		...
		oneMinusC := new(nogoec.FieldVal).Add(&c).Negate(1).AddInt(1)
		t := new(nogoec.FieldVal).Add(u).Mul(oneMinusC).Mul(twoInv).Add(v).
	...
	case 1:
		...
		onePlusC := new(nogoec.FieldVal).Add(&c).AddInt(1)
		t := new(nogoec.FieldVal).Add(u).Mul(onePlusC).Mul(twoInv).Add(v).
			Mul(w).Normalize()
...
func XElligatorSwift(x *nogoec.FieldVal) (*nogoec.FieldVal, *nogoec.FieldVal,
	error) {
...
func EllswiftCreate() (*nogoec.PrivateKey, [64]byte, error) {
	...
	privKey, _ := nogoec.PrivKeyFromBytes(randPrivKeyBytes[:])
...
func EllswiftECDHXOnly(ellswiftTheirs [64]byte, privKey *nogoec.PrivateKey) (
	[32]byte, error) {
	...
	var uTheirs nogoec.FieldVal
	overflow := uTheirs.SetByteSlice(uBytesTheirs[:])
	if overflow {
		uTheirs.Normalize()
	}

	var tTheirs nogoec.FieldVal
	...
	var pubJacobian nogoec.JacobianPoint
	pubKey.AsJacobian(&pubJacobian)

	var sharedPoint nogoec.JacobianPoint
	nogoec.ScalarMultNonConst(&privKey.Key, &pubJacobian, &sharedPoint)
...
// private key.
func getXCoord(privKey *nogoec.PrivateKey) *nogoec.FieldVal {
	var result nogoec.JacobianPoint
	nogoec.ScalarBaseMultNonConst(&privKey.Key, &result)
	result.ToAffine()
	return &result.X
}
...
func liftX(x *nogoec.FieldVal) (*nogoec.PublicKey, bool) {
	ySqr := new(nogoec.FieldVal).Add(x).Square().Mul(x).AddInt(7)

	y := new(nogoec.FieldVal)
	...
	if !y.Normalize().IsOdd() {
		return nogoec.NewPublicKey(x, y), true
	}
...
func V2Ecdh(priv *nogoec.PrivateKey, ellswiftTheirs, ellswiftOurs [64]byte,
