...
func TestSignatureSerialize(t *testing.T) {
	tests := []struct {
		...
		{
			"zero signature",
			NewSignature(&nogoec.ModNScalar{}, &nogoec.ModNScalar{}),
...
func testSignCompact(t *testing.T, tag string, curve *nogoec.KoblitzCurve,
	data []byte, isCompressed bool) {
	priv, _ := nogoec.NewPrivateKey()
...
func TestSignCompact(t *testing.T) {
	for i := 0; i < 256; i++ {
		...
		compressed := i%2 != 0
		testSignCompact(t, name, nogoec.S256(), data, compressed)
	}
}
...
func TestRecoverCompact(t *testing.T) {
	for i, test := range recoveryTests {
		...
		exPub, _ := nogoec.ParsePubKey(decodeHex(test.pub))
...
func TestRFC6979(t *testing.T) {
	...
	for i, test := range tests {
		privKey, _ := nogoec.PrivKeyFromBytes(decodeHex(test.key))
		hash := sha256.Sum256([]byte(test.msg))

		// Ensure deterministically generated nonce is the expected value.
		gotNonce := nogoec.NonceRFC6979(privKey.Serialize(), hash[:], nil, nil, 0).Bytes()
...
func testSignAndVerify(t *testing.T, c *nogoec.KoblitzCurve, tag string) {
	priv, _ := nogoec.NewPrivateKey()
	pub := priv.PubKey()
...
func TestSignAndVerify(t *testing.T) {
	testSignAndVerify(t, nogoec.S256(), "S256")
}
...
func TestPrivKeys(t *testing.T) {
	...
	for _, test := range tests {
		priv, pub := nogoec.PrivKeyFromBytes(test.key)

		_, err := nogoec.ParsePubKey(pub.SerializeUncompressed())
		if err != nil {
