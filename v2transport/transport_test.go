...
import (
	...
	"github.com/nogochain/nogocommons/nogoec/ellswift"
)

func setHex(hexString string) *nogoec.FieldVal {
	if len(hexString)%2 != 0 {
		hexString = "0" + hexString
	}
	bytes, _ := hex.DecodeString(hexString)

	var f nogoec.FieldVal
...
func TestPacketEncodingVectors(t *testing.T) {
	...
	for _, test := range tests {
		...
		var inPrivOursScalar nogoec.ModNScalar
		overflow := inPrivOursScalar.SetBytes(inPrivOursBytes)
		if overflow == 1 {
			t.Fatalf("unexpected reduction")
		}

		var inPubOurs nogoec.JacobianPoint
		nogoec.ScalarBaseMultNonConst(&inPrivOursScalar, &inPubOurs)
		inPubOurs.ToAffine()
		...
		var (
			uEllswiftOursFV nogoec.FieldVal
			tEllswiftOursFV nogoec.FieldVal
		)
		...
		var (
			uEllswiftTheirsFV nogoec.FieldVal
			tEllswiftTheirsFV nogoec.FieldVal
		)
		...
		privKeyOurs, _ := nogoec.PrivKeyFromBytes((*inPrivOursBytes)[:])
